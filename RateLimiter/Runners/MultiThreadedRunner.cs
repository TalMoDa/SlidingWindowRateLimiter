...
{
    // This method demonstrates how to use the WindowRateLimiter class to rate-limit calls to a function with multiple threads.
    public async Task Run(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var tasks = Enumerable.Range(0, 9999).Select(i => limiter.PerformAsync(i, cancellationToken));
    ...
    // This method demonstrates how to use the MemoryCacheRateLimiter class to rate-limit calls to a function with multiple threads.
    public async Task RunWithMemoryCache(CancellationToken cancellationToken = default)
    {
        Console.WriteLine("Starting MemoryCacheRateLimiter Rate-Limited Calls...");

        // Create a new WindowRateLimiter instance
        await using var limiter = new MemoryCacheRateLimiter<int>(
            // the function to be rate-limited (in this case, a simple async lambda that writes to the console)
            i =>
            {
                // Write to the console to demonstrate that the function is being executed
        ...
        try
        {
            ...
            Console.WriteLine("Completed  MemoryCacheRateLimiter Rate-Limited Calls.");
